...
const OnboardingRole = () => {
  ...
  const handleNext = async () => {
    try {
      const res = await axios.post(
        `${URL_ORIGIN}/auth/updateProfile/setRole`,
        { role },
        {
      ...
      if (role === "Writer") {
        navigate({ to: "/onboarding/setUpBlog" });
      } else {
  ...
  return (
      ...
      <div className="flex flex-col gap-10 w-fit px-16 py-12">
        ...
        <div className="flex flex-col gap-4 py-8 border-y-2 border-gray-400 dark:border-gray-500">
          ...
          <div className="flex flex-col gap-4">
            <button
              className={`px-8 py-4 rounded-md flex gap-4 items-center border
   ...
   `}
            ...
            <button
              className={`px-8 py-4 rounded-md flex gap-4 items-center border ${role === "Reader" ? "border-accent-dark dark:border-accent-dark" : "border-gray-400 dark:border-gray-500"}`}
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <p className="font-content text-neutral-600 dark:text-neutral-400">
                  I am here to read article and be a part of the community
                </p>
                <p className="font-content text-neutral-600 dark:text-neutral-400">
                  (You can always start your blog later)
                </p>
              </div>
            </button>
          </div>
          <div className="flex justify-end">
            <ButtonFill onClick={handleNext}>Next</ButtonFill>
          </div>
        </div>
      </div>
    </div>
